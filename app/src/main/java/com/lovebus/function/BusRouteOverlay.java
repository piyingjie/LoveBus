 ...
 * @since V2.1.0
 */
public class BusRouteOverlay extends com.lovebus.function.RouteOverlay{
	...
	public BusRouteOverlay(Context context, AMap amap, BusPath path,
		...
		super(context);
		this.busPath = path;
		startPoint = LoveBusUtil.convertToLatLng(start);
		endPoint = LoveBusUtil.convertToLatLng(end);
		mAMap = amap;
	}
		...
		try {
			...
			for (int i = 0; i < busSteps.size(); i++) {
				...
				if (busStep.getBusLine() != null) {
					...
					if (i == busSteps.size() - 1) {
						addWalkPolyline(LoveBusUtil.convertToLatLng(getLastBuslinePoint(busStep)), endPoint);
					}
				}
				if (busStep.getRailway() != null) {
					...
					if (i == busSteps.size() - 1) {
						addWalkPolyline(LoveBusUtil.convertToLatLng(busStep.getRailway().getArrivalstop().getLocation()), endPoint);
					}
				}
	...
	private void addRailwayStep(RouteRailwayItem railway) {
		...
		for (int i = 0; i < railwayStationItems.size(); i++) {
			railwaylistpoint.add(LoveBusUtil.convertToLatLng(railwayStationItems.get(i).getLocation()));
		}
	...
	private void addTaxiStep(TaxiItem taxi){
		addPolyLine(new PolylineOptions().width(getRouteWidth())
				.color(getBusColor())
				.add(LoveBusUtil.convertToLatLng(taxi.getOrigin()))
				.add(LoveBusUtil.convertToLatLng(taxi.getDestination())));
	}
	...
	private void addWalkSteps(BusStep busStep) {
		...
		for (int j = 0; j < walkSteps.size(); j++) {
			...
			if (j == 0) {
				LatLng latLng = LoveBusUtil.convertToLatLng(walkStep
						.getPolyline().get(0));
			...
			List<LatLng> listWalkPolyline = LoveBusUtil
			...
			if (j < walkSteps.size() - 1) {
				LatLng lastLatLng = listWalkPolyline.get(listWalkPolyline
						.size() - 1);
				LatLng firstlatLatLng = LoveBusUtil
	...
	private void addBusLineSteps(List<LatLonPoint> listPoints) {
		...
		addPolyLine(new PolylineOptions().width(getRouteWidth())
				.color(getBusColor())
				.addAll(LoveBusUtil.convertArrList(listPoints)));
	}
	...
	private void addBusStationMarkers(RouteBusLineItem routeBusLineItem) {
		...
		LatLng position = LoveBusUtil.convertToLatLng(startBusStation
				.getLatLonPoint());
	...
	private void addTaxiMarkers(TaxiItem taxiItem) {
		...
		LatLng position = LoveBusUtil.convertToLatLng(taxiItem
				.getOrigin());
	...
	private void addRailwayMarkers(RouteRailwayItem railway) {
		LatLng Departureposition =LoveBusUtil.convertToLatLng(railway
		...
		LatLng Arrivalposition =LoveBusUtil.convertToLatLng(railway
	...
	private void checkBusToNextBusNoWalk(BusStep busStep, BusStep busStep1) {
		LatLng endbusLatLng = LoveBusUtil
				.convertToLatLng(getLastBuslinePoint(busStep));
		LatLng startbusLatLng = LoveBusUtil
	...
	private void checkBusEndToNextBusStart(BusStep busStep, BusStep busStep1) {
		LatLonPoint busLastPoint = getLastBuslinePoint(busStep);
		LatLng endbusLatLng = LoveBusUtil.convertToLatLng(busLastPoint);
		LatLonPoint busFirstPoint = getFirstBuslinePoint(busStep1);
		LatLng startbusLatLng = LoveBusUtil.convertToLatLng(busFirstPoint);
	...
	private void addWalkPolyLineByLatLonPoints(LatLonPoint pointFrom,
			LatLonPoint pointTo) {
		LatLng latLngFrom = LoveBusUtil.convertToLatLng(pointFrom);
		LatLng latLngTo = LoveBusUtil.convertToLatLng(pointTo);
